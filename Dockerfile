...
RUN echo "===== Scouter Configuration =====" \
    ...
    && echo "cd /home/appuser && java -XX:StringTableSize=1000001 -javaagent:/home/appuser/scouter/agent.java/scouter.agent.jar -Dscouter.config=/home/appuser/scouter/agent.java/conf/scouter.conf -Dobj_name=jpetstore_`hostname` -Djava.security.egd=file:/dev/./urandom -Dspring.profiles.active=\${PROFILE} -jar app.war" >> run.sh
